...
async function refreshCreditCardBySource(sourceId) {
  ...
  try {
    const {
      getCreditCardBySourceId,
      refreshCreditCardTotals,
      syncCreditCardBillAmount,
    } = require("./creditCards");
...
export async function createTransaction(tx) {
  ...
  const principal_component =
    tx.principal_component != null ? tx.principal_component : null;
  const interest_component =
    tx.interest_component != null ? tx.interest_component : null;
  const outstanding_after_payment =
    tx.outstanding_after_payment != null ? tx.outstanding_after_payment : null;
...
export async function getTransactions(
  ...
  try {
    ...
    const filtered = rows.filter((row) => {
      if (isTransferInclude === "No" && row.transfer_group_id !== null)
        return false;
  ...
  try {
    if (Platform.OS === "web") {
      ...
      for (let i = 0; i < txRes.rows.length; i++)
        transactions.push(txRes.rows.item(i));
      ...
      for (let i = 0; i < srcRes.rows.length; i++)
        sources.push(srcRes.rows.item(i));

      let result = transactions.map((t) => ({
        ...t,
        category_name: categories.find(
          (c) => String(c.id) === String(t.category_id),
        )?.name,
        source_name: sources.find((s) => String(s.id) === String(t.source_id))
          ?.name,
      }));
      ...
      if (filterType === "expense") {
        ...
        result = result.filter(
          (t) =>
            t.type === "transfer" ||
            t.transfer_group_id !== null ||
            t.is_transfer === 1,
        );
      }

      if (sourceId !== null && sourceId !== undefined) {
        result = result.filter((t) => Number(t.source_id) === Number(sourceId));
      }
...
export async function getSourceTransactionBalance(sourceId) {
  try {
    if (Platform.OS === "web") {
      const res = await executeSql(
        "SELECT amount, type FROM transactions WHERE source_id = ?",
        [sourceId],
      );
      let total = 0;
      for (let i = 0; i < res.rows.length; i++) {
        ...
        total +=
          row.type === "income" ? Number(row.amount) : -Number(row.amount);
      }
      return total;
    }
    ...
    if (res.rows.length > 0) {
      return Number(res.rows.item(0).total || 0);
    }
    return 0;
  } catch (error) {
...
export async function updateTransaction(id, fields) {
  ...
  try {
    events.emit("transactionsChanged", { action: "update", id, fields });
  } catch (e) {}
...
export async function getSourceTransactionBalances() {
  try {
    ...
    console.error("getSourceTransactionBalances error:", error);
...
export async function getCategoryAndSourceUsage(years = 1) {
  ...
  try {
    ...
    const categoryCount = {};
    for (let i = 0; i < catRes.rows.length; i++) {
      categoryCount[String(catRes.rows.item(i).category_id)] =
        catRes.rows.item(i).count;
    }

    const sourceCount = {};
    for (let i = 0; i < srcRes.rows.length; i++) {
      sourceCount[String(srcRes.rows.item(i).source_id)] =
        srcRes.rows.item(i).count;
    }
  ...
  try {
    if (Platform.OS === "web") {
      ...
      const filtered = rows.filter((tx) => {
        if (categoryId && Number(tx.category_id) !== Number(categoryId))
          return false;
