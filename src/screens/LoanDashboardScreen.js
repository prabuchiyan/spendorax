...
export default function LoanDashboardScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    const enriched = data.map((l) => ({
      ...l,
      nextEmiDate: computeNextEmiDate(l),
      isOverdue:
        l.outstanding_amount > 0 &&
        l.loan_end_date &&
        new Date(l.loan_end_date) < new Date(),
    }));
    setLoans(enriched);
  ...
  const portfolio = useMemo(() => {
    const borrowedLoans = loans.filter(
      (l) => (l.loan_direction || "BORROWED") === "BORROWED",
    );
    const lentLoans = loans.filter(
      (l) => (l.loan_direction || "BORROWED") === "LENT",
    );

    const borrowedOutstanding = borrowedLoans.reduce(
      (s, l) => s + Number(l.outstanding_amount || 0),
      0,
    );
    const pendingRecovery = lentLoans.reduce(
      (s, l) => s + Number(l.outstanding_amount || 0),
      0,
    );
  ...
  const health = useMemo(() => {
    ...
    const paid = borrowed.reduce(
      (s, l) =>
        s + (Number(l.principal_paid || 0) + Number(l.interest_paid || 0)),
      0,
    );
    const total = borrowed.reduce(
      (s, l) =>
        s +
        (Number(l.principal_paid || 0) +
          Number(l.interest_paid || 0) +
  ...
  const upcoming = useMemo(() => {
    const arr = loans
      ...
      .map((l) => ({ ...l, nextDate: new Date(l.nextEmiDate) }));
  ...
  const activeLoans = useMemo(
    () =>
      loans.filter(
        (l) =>
          Number(l.outstanding_amount || 0) > 0 &&
          (l.status || "Active") === "Active",
      ),
    [loans],
  );
  const borrowedActiveLoans = useMemo(
    () =>
      activeLoans.filter(
        (l) => (l.loan_direction || "BORROWED") === "BORROWED",
      ),
  ...
  const lentActiveLoans = useMemo(
    () =>
      activeLoans.filter((l) => (l.loan_direction || "BORROWED") === "LENT"),
    [activeLoans],
  );
    ...
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <LoanSummaryCard
            title="Borrowed Outstanding"
            amount={portfolio.borrowedOutstanding}
            icon="bank"
          />
          <LoanSummaryCard
            title="Pending Recovery"
            amount={portfolio.pendingRecovery}
            icon="cash-refund"
          />
        </View>
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <LoanSummaryCard
            title="Net Position"
            amount={portfolio.netPosition}
            icon="scale-balance"
          />
          <LoanSummaryCard
            title="Monthly EMI"
            amount={summary.totalEMI}
            icon="calendar-month"
          />
        </View>
        ...
        <View style={{ marginTop: 20 }}>
          <LoanQuickActions
            ...
            onAll={() => navigation.navigate("LoanList")}
          />
        </View>
        ...
        <View style={{ marginTop: 20 }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {upcoming.length === 0 ? (
              <Card style={{ padding: 0, marginLeft: 1, marginTop: 1 }}>
        ...
        <View style={{ marginTop: 20 }}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate("LoanDetails", { id: item.id })
                }
              >
