  ...
  return (
    <Card style={{ borderRadius: 20, overflow: 'hidden' }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <View>
          <Text style={{ fontSize: 17, fontWeight: '800', color: Colors.text }}>Linked Transactions</Text>
          <Text style={{ fontSize: 12, color: Colors.muted, marginTop: 2 }}>
            {linkedTxs.length} transaction{linkedTxs.length !== 1 ? 's' : ''}
          </Text>
        </View>
        {onAddMore && (
          <TouchableOpacity onPress={onAddMore} style={{
      ...
      {linkedTxs.length === 0 ? (
        <View style={{ paddingVertical: 40, alignItems: 'center' }}>
          ...
          <Text style={{ marginTop: 8, fontWeight: '700', color: Colors.muted }}>No linked transactions</Text>
          <Text style={{ marginTop: 4, fontSize: 12, color: Colors.muted, textAlign: 'center' }}>
            Link a payment to automatically mark this bill as paid.
          </Text>
        </View>
      ) : (
        linkedTxs.map(tx => (
          <View key={tx.id} style={{ marginBottom: 10, borderRadius: 16, backgroundColor: '#FAFAFA', padding: 14 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{
                ...
                <MaterialCommunityIcons name={tx.category_icon || 'cash'} color={tx.category_color || Colors.primary} size={24} />
              </View>
              <View style={{ flex: 1, marginLeft: 12 }}>
                <Text numberOfLines={1} style={{ fontSize: 15, fontWeight: '700', color: Colors.text }}>
                  {tx.notes || 'No Notes'}
                </Text>
                <Text style={{ marginTop: 3, fontSize: 12, color: Colors.muted }}>
                  {tx.date ? new Date(tx.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                  {tx.source_name ? ` • ${tx.source_name}` : ''}
                </Text>
                <Text style={{ marginTop: 6, fontSize: 17, fontWeight: '800', color: Colors.text }}>
                  {formatCurrency(tx.amount)}
                </Text>
              </View>
              {onUnlink && (
                <TouchableOpacity onPress={() => onUnlink(tx)} style={{
                  width: 42, height: 42, borderRadius: 21,
                  backgroundColor: '#FFECEC', justifyContent: 'center', alignItems: 'center',
                }}>
                  <MaterialCommunityIcons name="link-variant-remove" size={22} color="#F44336" />
                </TouchableOpacity>
              )}
            </View>
          </View>
  ...
  return (
    <Card style={{ borderRadius: 20 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
        <View>
          <Text style={{ fontWeight: '800', fontSize: 17, color: Colors.text }}>Timeline</Text>
          <Text style={{ fontSize: 12, color: Colors.muted, marginTop: 2 }}>{series.length} Bill Occurrences</Text>
        </View>
        ...
        {series.map(occ => {
          ...
          return (
            <TouchableOpacity
              ...
              style={{
                width: 90, marginRight: 10, borderRadius: 18,
              ...
              <MaterialCommunityIcons
                name={
                  display.label === 'Paid' ? 'check-circle' :
                    display.label === 'Skipped' ? 'skip-next-circle' :
                      display.label === 'Overdue' ? 'alert-circle' : 'clock-outline'
                }
                size={18}
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' }}>
        <View style={{ backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, maxHeight: '72%', padding: 16 }}>
          ...
          <Text style={{ color: Colors.muted, fontSize: 13, marginBottom: 12 }}>
            Showing {bill?.due_date?.slice(0, 7)} transactions for "{bill?.name}"
          </Text>
          {loading ? (
            ...
            <FlatList
              ...
              renderItem={({ item: tx }) => (
                <TouchableOpacity
                  onPress={() => onLink(tx)}
                  style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#EEF1F6' }}
                >
                  <View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: `${tx.category_color || Colors.primary}20`, alignItems: 'center', justifyContent: 'center', marginRight: 10 }}>
                    <MaterialCommunityIcons name={tx.category_icon || 'cash'} size={18} color={tx.category_color || Colors.primary} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontWeight: '600', color: Colors.text, fontSize: 14 }} numberOfLines={1}>{tx.notes || '(no notes)'}</Text>
                    <Text style={{ color: Colors.muted, fontSize: 12, marginTop: 2 }}>
                      {tx.date ? new Date(tx.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                      {tx.source_name ? ` · ${tx.source_name}` : ''}
                    </Text>
                  </View>
                  <Text style={{ fontWeight: '800', color: '#E46A6A', marginLeft: 8, fontSize: 15 }}>{formatCurrency(tx.amount)}</Text>
                </TouchableOpacity>
              )}
            />
          )}
          <PaperButton mode="outlined" onPress={onClose} style={{ marginTop: 8 }}>Cancel</PaperButton>
        </View>
      </View>
    </Modal>
  );
          ...
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', padding: 16 }}>
            <View style={{ backgroundColor: '#fff', borderRadius: 12, padding: 16 }}>
              ...
              <ManualDateTimePicker
                ...
                hour={0} minute={0}
                onChange={(y, m, d) => setDueDate(`${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}`)}
...
export default function BillDetailScreen({ route, navigation }) {
  ...
  async function refreshSelectedOccurrence() {
    ...
    const occ = updatedSeries.find(o => Number(o.id) === Number(selectedOcc?.id)) || null;
    setSelectedOcc(occ);
  ...
  const isCurrentMonth = activeBill.due_date?.slice(0, 7) === currentMonthStr;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      >
        ...
        {/* Hero card */}
        <Card style={{ marginBottom: 12, borderRadius: 22, overflow: 'hidden', padding: 0 }}>
          <View style={{ backgroundColor: `${display.color}12`, padding: 18, flexDirection: 'row', alignItems: 'center' }}>
            <View style={{ width: 64, height: 64, borderRadius: 32, backgroundColor: `${display.color}25`, justifyContent: 'center', alignItems: 'center' }}>
              <MaterialCommunityIcons name={category?.icon || 'receipt'} size={30} color={display.color} />
            </View>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', padding: 14 }}>
            {[
              { icon: 'calendar', label: 'Due', value: formatDueDate(activeBill.due_date) },
              { icon: category?.icon || 'shape', label: 'Category', value: category?.name || '-' },
              { icon: 'bank', label: 'Source', value: source?.name || '-' },
              { icon: 'repeat', label: 'Repeat', value: bill.is_recurring ? `${bill.recurrence_interval || 1} ${bill.recurrence_type}` : 'No' },
            ].map(({ icon, label, value }) => (
              <View key={label} style={styles.infoTile}>
                <MaterialCommunityIcons name={icon} color={Colors.primary} size={18} />
      ...
      <View style={styles.bottomBar}>
        {!isPaidOrSkipped && (
          <TouchableOpacity
            style={[styles.actionButton, { backgroundColor: '#2DBE60', opacity: isCurrentMonth ? 1 : 0.45 }]}
            disabled={!isCurrentMonth}
            onPress={handleMarkPaid}
          >
            <MaterialCommunityIcons name="check-circle" color="#fff" size={22} />
        ...
        {activeBill.status === BILL_STATUS.SKIPPED ? (
          ...
          <TouchableOpacity style={styles.actionButton} onPress={() => { setConfirmAction('skip'); setConfirmVisible(true); }}>
            <MaterialCommunityIcons name="skip-next-circle" color="#F57C00" size={22} />
            <Text style={styles.actionText}>Skip</Text>
          </TouchableOpacity>
        ) : null}

        <TouchableOpacity style={styles.actionButton} onPress={() => setShowLinkModal(true)}>
          <MaterialCommunityIcons name="link-variant" color={Colors.primary} size={22} />
...
const styles = StyleSheet.create({
  ...
  infoValue: { marginTop: 2, fontWeight: '700', color: Colors.text, fontSize: 14 },
  bottomBar: {
